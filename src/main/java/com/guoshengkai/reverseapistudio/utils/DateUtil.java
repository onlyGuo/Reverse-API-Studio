...
public class DateUtil {
     ...
     * 将不带日期的纯时间字符串转换为毫秒值
     * @param trim
     *      时间字符串，格式为 "HH:mm:ss.SSS" 或 "HH:mm:ss"
     * @return
     *      毫秒值
     */
    public static long parseTimeToMillis(String trim) {
        // 00:00.000
        if (trim == null || trim.isEmpty()) {
            return 0;
        }
        // 检查有几个冒号，少的话补齐
        StringBuilder trimBuilder = new StringBuilder(trim);
        while (trimBuilder.chars().filter(ch -> ch == ':').count() < 2) {
            trimBuilder.insert(0, "00:");
        }
        trim = trimBuilder.toString();
        ...
        try {
            ...
            return (hours * 3600L + minutes * 60L) * 1000 + millis;
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Invalid time format: " + trim, e);
        }
    }
}
