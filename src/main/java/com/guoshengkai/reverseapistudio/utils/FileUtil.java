...
import com.guoshengkai.reverseapistudio.entitys.models.*;
...
public class FileUtil {
     ...
     * @return
     */
    public static ModelEntity parseMds(String content) {
        ...
        modelEntity.setFields(new ArrayList<>());
        ...
        for (int i = 0; i < content.length(); i++) {
            ...
            if (!inBlockComment && !inLineComment && !inString && !inRegex) {
                if (c == '/' && content.charAt(i + 1) == '/') {
                    inLineComment = true;
                    continue;
                }
        ...
        if (leftBrace == -1) {
            throw new RuntimeException("没有发现完整的模型定义");
        }

        // 3. 判断是否已字母开始
        if(!Character.isLetter(line.charAt(0))){
            throw new RuntimeException("必须指定一个由英文字母开头的模型名称: " + line);
        }
        // 4. 解析模型名称
        ...
        if(left != -1){
            ...
            if(right == -1){
                throw new RuntimeException("模型名称括号不匹配: " + line);
            }
            String name = line.substring(0, left).trim();
            // 判断名称是否是大小写字母组成
            if(!name.matches("[a-zA-Z]+")){
                throw new RuntimeException("模型名称必须是大小写字母组成: " + line);
            }
            modelEntity.setName(name);
            String[] params = line.substring(left + 1, right).split(",");
            if (params.length == 1) {
                // 如果有一个参数，那么视为描述
                modelEntity.setDescription(parseModelParam(modelEntity, line, params[0].trim()));
            }else if (params.length == 2) {
                // 如果有两个参数，那么第一个参数是表名，第二个参数是描述
                modelEntity.setTableName(parseModelParam(modelEntity, line, params[0].trim()));
                modelEntity.setDescription(parseModelParam(modelEntity, line, params[1].trim()));
            }
        }else{
            // 如果没有参数，那么就是非入库字段
        ...
        if(sb.charAt(sb.length() - 1) != '}'){
            throw new RuntimeException("没有发现模型边界符号，检查花括号是否匹配: " + sb);
        }

        // 5. 解析模型中的字段
        String[] split = sb.substring(leftBrace + 1, sb.length() - 1).split(";");
        for (String s : split) {
            s = s.trim();
            if (s.length() == 0) {
                continue;
            ...
            // 1. 先判断字段中是否存在参数
            int fieldLeft = s.indexOf('(');
            if(fieldLeft != -1){
                // 如果有参数
                int fieldRight = s.indexOf(')', fieldLeft);
                if(fieldRight == -1){
                    throw new RuntimeException("字段参数不匹配,缺少右括号: " + s);
                }
                paramString = s.substring(fieldLeft, fieldRight + 1);
                params = paramString.substring(1, paramString.length() - 1).split(",");
            }

            String temp = s;
            if (paramString != null && paramString.length() > 0) {
                temp = s.replace(paramString, " ");
            }
            // 2. 判断是否是必须字段
            required = s.startsWith("require");
            // 拿到每个关键字
            String[] keywords = temp.split("\\s+");
            // 判断类型是否合法
            if (required){
                if(keywords.length < 3){
                    // 如果没有类型，那么报错
                    throw new RuntimeException("滥用的关键字: " + s);
                }
                // 第二个元素是字段类型
                type = FieldType.parse(keywords[1]);
                if (type == null) {
                    throw new RuntimeException("字段类型[" +  keywords[1] + "]不合法: " + s);
                }
                // 第三个是字段名
                fieldName = keywords[2];
                // 判断第四个是冒号还是中括号，如果是冒号，则第4个是正则表达式，如果是中括号，则第4个是校验规则
//                if(keywords.length < 4){
//                    throw new RuntimeException("字段[" + fieldName + "]没有正则表达式或校验规则: " + s);
//                }
                if (keywords.length >= 4) {
                    if(keywords[3].equals(":")){
                        // 如果是冒号，那么第四个是正则表达式
                        regex = keywords[4];
                    }else if(keywords[3].equals("[")){
                        // 如果是中括号，那么第四个是校验规则
                        if (!temp.endsWith("]")) {
                            throw new RuntimeException("字段[" + fieldName + "]校验规则括号不匹配: " + s);
                        }
                        rules = temp.substring(temp.indexOf("[") + 1, temp.lastIndexOf("]")).trim();
                        rules.substring(0, rules.length() - 1).trim();
                    }else{
                        throw new RuntimeException("字段[" + fieldName + "]校验规则不合法: " + s);
                    }
                }
            }else{
                ...
                // 第一个元素是字段类型
                type = FieldType.parse(keywords[0]);
                if (type == null) {
                    throw new RuntimeException("字段类型[" +  keywords[0] + "]不合法: " + s);
                }
                // 第二个是字段名
                fieldName = keywords[1];
                // 判断第三个是冒号还是中括号，如果是冒号，则第4个是正则表达式，如果是中括号，则第4个是校验规则
//                if(keywords.length < 3){
//                    throw new RuntimeException("字段[" + fieldName + "]没有正则表达式或校验规则: " + s);
//                }
                if(keywords.length >= 3){
            ...
            field.setType(type.getScriptType());
            field.setName(fieldName);
            field.setRequired(required);
            if (regex != null) {
                if (regex.startsWith("/") && regex.endsWith("/")) {
                    field.setRegex(regex.substring(1, regex.length() - 1));
                }else{
                    try {
                        field.setRegex(ValidationType.valueOf(regex).getRegex());
                    }catch (Exception e){
                        throw new RuntimeException("字段[" + fieldName + "]没有对应的内置正则[" + regex + "]: " + s);
                    }
                }
                try {
                    // 校验正则合法性
                    ...
                    throw new RuntimeException("字段[" + fieldName + "]正则表达式[" + field.getRegex() + "]不合法: " + s);
                }
            }
            ...
            if (params != null && params.length > 0) {
                if (params.length == 1) {
                    ...
                    if (params[0].startsWith("'") && params[0].endsWith("'")) {
                        field.setComment(params[0].substring(1, params[0].length() - 1));
                    }else{
                        throw new RuntimeException("字段[" + fieldName + "]参数列表格式不合法: " + s);
                    }
                }else{
                    params[0] = params[0].trim();
                    params[1] = params[1].trim();
                    if (params[0].startsWith("'") && params[0].endsWith("'")) {
                        field.setTableFieldName(params[0].substring(1, params[0].length() - 1));
                    }else{
                        throw new RuntimeException("字段[" + fieldName + "]参数列表格式不合法: " + s);
                    }
                    if (params[1].startsWith("'") && params[1].endsWith("'")) {
                        field.setComment(params[1].substring(1, params[1].length() - 1));
                    }else{
                        throw new RuntimeException("字段[" + fieldName + "]参数列表格式不合法: " + s);
                    }
                }
            ...
            if (rules != null) {
                List<ModelFieldValidationRule> ruleList = new ArrayList<>();
                String[] rule = rules.split(",");
                for (String r : rule) {
                    String[] item = r.trim().split(":");
                    if (item.length != 2) {
                        throw new RuntimeException("字段[" + fieldName + "]校验规则不合法: " + s);
                    }
                    ModelFieldValidationRule mfvr = new ModelFieldValidationRule();
                    String ruleName = item[0].trim();
                    if (ruleName.equals("scope")) {
                        String[] split1 = item[1].trim().split("-");
                        if (split1.length != 2) {
                            throw new RuntimeException("字段[" + fieldName + "]Scope规则不合法: " + s);
                        }
                        try {
                            mfvr.setMinValue(Double.parseDouble(split1[0].trim()));
                            mfvr.setMaxValue(Double.parseDouble(split1[1].trim()));
                        }catch (Exception e){
                            throw new RuntimeException("字段[" + fieldName + "]Scope规则不合法: " + s);
                        }
                    }
                    if (ruleName.equals("length")) {
                        String[] split1 = item[1].trim().split("-");
                        if (split1.length != 2) {
                            throw new RuntimeException("字段[" + fieldName + "]Length规则不合法: " + s);
                        }
                        try {
                            mfvr.setMinLength(Integer.parseInt(split1[0].trim()));
                            mfvr.setMaxLength(Integer.parseInt(split1[1].trim()));
                        }catch (Exception e){
                            throw new RuntimeException("字段[" + fieldName + "]Length规则不合法: " + s);
                        }
                    }
                ...
                field.setValidationRules(ruleList);
            }

            modelEntity.getFields().add(field);
        }
        return modelEntity;
    }

    private static String parseModelParam(ModelEntity modelEntity, String line, String params) {
        if (params.startsWith("'") && params.endsWith("'")) {
            params = params.substring(1, params.length() - 1);
        }else{
            throw new RuntimeException("模型参数必须是单引号包裹的字符串: " + line);
        }
        return params;
    }
