...
export default {
  ...
  methods: {
    close() {
      ...
      setTimeout(() => {
        delete this.$editor.confirmWindowList[this.option.id];
      }, 250);
      this.option.callback(false);
    },
    onMousedown(e) {
    ...
    shake() {
      // 先将窗口处于最顶层
      this.$refs.box.style.zIndex = this.$editor.confirmWindowList.length;
      // 开始抖动
