  ...
  <div ref="box" class="box" :style="'height: ' + option.height + ';width: ' +
    ...
    <div class="prompt__body">
      <div style="height: 50px; line-height: 50px; padding-left: 20px">
        <input type="text" :placeholder="option.placeholder" v-model="option.value" @keyup.enter="confirm" style="width: 290px"/>
      </div>
    </div>
...
export default {
  ...
  methods: {
    close() {
      ...
      setTimeout(() => {
        delete this.$editor.promptWindowList[this.option.id];
      }, 250);
    },
    onMousedown(e) {
    ...
    shake() {
      // 先将窗口处于最顶层
      this.$refs.box.style.zIndex = this.$editor.promptWindowList.length;
      // 开始抖动
